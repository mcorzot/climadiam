---
title: "climadiam: Paquete R para consultar datos climáticos de Andalucía"
...
# Instalar 'remotes', 'ggplot2' y 'dplyr' si no están instalados
list.of.packages <- c("remotes","ggplot2","dplyr")
...
Primera versión del paquete **climadiam** para conectar con los
servicios web del Subsistema de Climatología Ambiental (REDIAM). Permite
consultar datos de estaciones meteorológicas de Andalucía de forma
sencilla desde R.

# 🌍 Descripción general

El paquete permite conectarse al Subsistema CLIMA y facilita el acceso a
datos meteorológicos y ambientales, incluyendo:
...
-   En caso de realizar trabajos para la Junta de Andalucía o como socio de la 
...
```{r examples_basic2, collapse=FALSE}
...
```{r example_full_dates_ayer, collapse=FALSE}
...
```{r example_full_dates_antes_de_ayer, collapse=FALSE}
# Obtención de fecha a utilizar
date2 <- Sys.Date() - 2
...
```{r example_full_est, collapse=FALSE}
...
```{r example_full_vars, collapse=FALSE}
...
```{r example_full_intra, collapse=TRUE, eval=TRUE}
...
```{r example_full_dia, collapse=TRUE, eval=TRUE}
# Datos diarios
